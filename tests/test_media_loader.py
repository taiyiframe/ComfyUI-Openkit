...
# P1修复: 硬编码旧版便携目录路径更新为当前运行时目录
nodes_dir = r"D:\ComfyUI-20260916-045343\ComfyUI\custom_nodes\ComfyUI-Openkit\nodes"
...
# P1修复: 后端 splitter 已把尾槽 None 替换为类型化哨兵(empty_audio dict / empty_video tensor)，
# 故尾槽不应再为 None。
# bundle has 2 audios → first two filled, rest typed sentinel
assert audios_out[0] is not None and audios_out[1] is not None
assert all(a is not None for a in audios_out[2:]), audios_out
# 2 videos filled
assert videos_out[0] is not None and videos_out[1] is not None
assert videos_out[2] is not None
# 1 paired soundtrack (only video 1 paired)
assert vauds_out[0] is not None
assert all(v is not None for v in vauds_out[1:]), vauds_out
...
# P1修复: 尾槽不再是裸 None，而是类型化哨兵(empty_audio dict / empty_video tensor)
assert all(o is not None for o in out2[4:]), out2[4:]
print("Test6 empty bundle -> 4 empty lists + media ports typed sentinels OK")
